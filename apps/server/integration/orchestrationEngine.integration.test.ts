    ...
    while (true) {
      ...
      if (Date.now() >= deadline) {
        return yield* Effect.die(
          new IntegrationWaitTimeoutError({ description }),
        );
      }
      yield* sleep(10);
    }
    ...
    withHarness((harness) =>
      Effect.gen(function* () {
        ...
        const checkpointRows =
          yield* harness.checkpointRepository.listByThreadId({
            threadId: THREAD_ID,
          });
        ...
        assert.equal(
          gitShowFileAtRef(harness.workspaceDir, ref0, "README.md"),
          "v1\n",
        );
        assert.equal(
          gitShowFileAtRef(harness.workspaceDir, ref1, "README.md"),
          "v1\n",
        );
      }),
    ),
      ...
      Effect.gen(function* () {
        ...
        const secondThread = yield* harness.waitForThread(
          ...
          (entry) =>
            ...
            entry.messages.some(
              (message) =>
                message.role === "assistant" && message.text.includes("BETA"),
            ),
          180_000,
        );
...
it.live(
  "tracks approval requests and resolves pending approvals on user response",
  () =>
    withHarness((harness) =>
      Effect.gen(function* () {
        yield* seedProjectAndThread(harness);
  ...
  () =>
    withHarness((harness) =>
      Effect.gen(function* () {
        ...
        const checkpointRow =
          yield* harness.checkpointRepository.getByThreadAndTurnCount({
            threadId: THREAD_ID,
      ...
      Effect.gen(function* () {
        ...
        yield* harness.adapterHarness!.queueTurnResponseForNextSession({
          events: [
            ...
            },
            {
              type: "tool.started",
              ...runtimeBase(
                "evt-revert-1-tool-started",
                "2026-02-24T10:05:00.025Z",
              ),
              threadId: THREAD_ID,
              turnId: FIXTURE_TURN_ID,
              toolKind: "command",
            ...
            },
            {
              type: "tool.completed",
              ...runtimeBase(
                "evt-revert-1-tool-completed",
                "2026-02-24T10:05:00.035Z",
              ),
              threadId: THREAD_ID,
              turnId: FIXTURE_TURN_ID,
              toolKind: "command",
        ...
        yield* harness.adapterHarness!.queueTurnResponse(THREAD_ID, {
          events: [
            ...
            },
            {
              type: "tool.started",
              ...runtimeBase(
                "evt-revert-3-tool-started",
                "2026-02-24T10:05:01.025Z",
              ),
              threadId: THREAD_ID,
              turnId: FIXTURE_TURN_ID,
              toolKind: "command",
            ...
            },
            {
              type: "tool.completed",
              ...runtimeBase(
                "evt-revert-3-tool-completed",
                "2026-02-24T10:05:01.035Z",
              ),
              threadId: THREAD_ID,
              turnId: FIXTURE_TURN_ID,
              toolKind: "command",
        ...
        assert.equal(
          revertedThread.activities.some(
            (activity) => activity.turnId === "turn-2",
          ),
          false,
        );
        assert.equal(
          revertedThread.activities.some(
            (activity) =>
              activity.turnId === "turn-1" && activity.kind === "tool.started",
          ),
          true,
        );
        assert.equal(
        ...
        assert.equal(
          fs.readFileSync(path.join(harness.workspaceDir, "README.md"), "utf8"),
          "v2\n",
        );
        assert.equal(
          gitRefExists(
...
it.live(
  "starts a claudeCode session on first turn when provider is requested",
  () =>
    withHarness(
      (harness) =>
        Effect.gen(function* () {
            ...
            (entry) =>
              ...
              entry.messages.some(
                (message) =>
                  ...
                  message.text === "Claude first turn.\n",
              ),
          );
          assert.equal(thread.session?.providerName, "claudeCode");
        }),
      "claudeCode",
    ),
);
...
it.live(
  "recovers claudeCode sessions after provider stopAll using persisted resume state",
  () =>
    withHarness(
      (harness) =>
        Effect.gen(function* () {
          ...
          const recoveredThread = yield* harness.waitForThread(
            ...
            (entry) =>
              ...
              entry.messages.some(
                (message) =>
                  message.role === "user" && message.text === "After restart",
              ) &&
              !entry.activities.some(
...
it.live("forwards claudeCode approval responses to the provider session", () =>
  withHarness(
    (harness) =>
      Effect.gen(function* () {
        yield* seedProjectAndThread(harness);
...
it.live("forwards thread.turn.interrupt to claudeCode provider sessions", () =>
  withHarness(
    (harness) =>
      Effect.gen(function* () {
        yield* seedProjectAndThread(harness);
...
it.live(
  "reverts claudeCode turns and rolls back provider conversation state",
  () =>
    withHarness(
      (harness) =>
        Effect.gen(function* () {
